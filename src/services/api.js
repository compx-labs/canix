...
const categorizeAssetType = (assetType, assetName) => {
  ...
  if (name.includes('usdc') || name.includes('usdt') || type.includes('stablecoin')) {
    return 'Stablecoin';
  }
  
  // Check for governance tokens
  if (name.includes('algo') && !name.includes('talgo') && !name.includes('galgo')) {
    return 'Governance';
  }
  
  // Check for yield bearing
  if (name.includes('galgo') || name.includes('talgo') || type.includes('yield')) {
    return 'Yield bearing';
  }
  
  // Check for RWA
  if (name.includes('planet') || name.includes('opul') || type.includes('rwa')) {
    return 'RWA';
  }
...
const formatRewardAssets = (rewardAssets) => {
  ...
  return rewardAssets.map(reward => reward.name || `Asset ${reward.assetId}`).join(', ');
};
